...
class _CreateProFormaPageState extends State<CreateProFormaPage> {
  // Document Details
  String _prefix = DocumentConstants.proFormaPrefixes.first;
  String _documentNumber = '';
  DateTime _documentDate = DateTime.now();
  DateTime? _dueDate;
  String _documentTitle = 'Pro Forma Invoice';
  double _discount = 0;
  ...
  void _showDocumentDetailsDialog() {
    showDialog(
      ...
      builder: (context) => DocumentDetailsDialog(
        ...
        prefixOptions: DocumentConstants.proFormaPrefixes,
        showDueDate: true,
        onSave: (data) {
          setState(() {
            _prefix = data['prefix'];
  ...
  Future<void> _saveProForma() async {
    ...
    final proForma = InvoiceEntity(
      id: DateTime.now().millisecondsSinceEpoch.toString(),
      invoiceNumber: '$_prefix-$_documentNumber',
      invoiceType: InvoiceType.proFormaInvoice,
    ...
    // Save pro forma (NO stock deduction - it's just an estimate)
    final success = await invoiceProvider.addInvoice(proForma);
    ...
    if (success) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Pro Forma Invoice saved successfully!'),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(invoiceProvider.errorMessage ?? 'Failed to save pro forma'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Create Pro Forma Invoice'),
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Pro Forma is a preliminary invoice (estimate). No stock will be deducted. Convert to Tax Invoice after confirmation.',
                    style: TextStyle(
                      fontSize: 11,
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if (_dueDate != null)
                            Text('Validity: ${DateFormat('dd MMM yyyy').format(_dueDate!)}'),
                          if (_discount > 0)
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          ...
                          _buildTotalRow('Total Tax (Estimated)', _calculateTotalTax()),
                          _buildTotalRow('Additional Charges', _calculateAdditionalCharges()),
                          const Divider(height: 24),
                          _buildTotalRow('Estimated Total', _calculateGrandTotal(), isBold: true),
                        ],
                      ),
                    ),
                  ),
