...
class _CreateQuotationPageState extends State<CreateQuotationPage> {
  // Document Details
  String _prefix = DocumentConstants.quotationPrefixes.first;
  String _documentNumber = '';
  DateTime _documentDate = DateTime.now();
  DateTime? _dueDate;
  String _documentTitle = 'Quotation';
  double _discount = 0;
  ...
  void _showDocumentDetailsDialog() {
    showDialog(
      ...
      builder: (context) => DocumentDetailsDialog(
        ...
        prefixOptions: DocumentConstants.quotationPrefixes,
        showDueDate: true,
        onSave: (data) {
          setState(() {
            _prefix = data['prefix'];
  ...
  Future<void> _saveQuotation() async{
    ...
    // Save quotation (NO stock changes - just a quote)
    final success = await invoiceProvider.addInvoice(quotation);
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(invoiceProvider.errorMessage ?? 'Failed to save quotation'),
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Quotation is a price quote for customer. No stock or payment changes.',
                    style: TextStyle(
                      fontSize: 11,
