...
class _CreatePurchasePageState extends State<CreatePurchasePage> {
  // Document Details
  String _prefix = DocumentConstants.purchaseBillPrefixes.first;
  String _documentNumber = '';
  DateTime _documentDate = DateTime.now();
  DateTime? _dueDate;
  String _documentTitle = 'Purchase Bill';
  double _discount = 0;
  ...
  void _showDocumentDetailsDialog() {
    showDialog(
      ...
      builder: (context) => DocumentDetailsDialog(
        ...
        prefixOptions: DocumentConstants.purchaseBillPrefixes,
        showDueDate: true,
        onSave: (data) {
          setState(() {
            _prefix = data['prefix'];
  ...
  Future<void> _savePurchase() async {
    ...
    // Save purchase
    final success = await invoiceProvider.addInvoice(purchase);
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(invoiceProvider.errorMessage ?? 'Failed to save purchase'),
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Purchase Bill increases your inventory and creates payable to supplier.',
                    style: TextStyle(
                      fontSize: 11,
